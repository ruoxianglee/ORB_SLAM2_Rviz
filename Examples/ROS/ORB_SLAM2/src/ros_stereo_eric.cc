...
public:
    ImageGrabber(ORB_SLAM2::System* pSLAM, ORB_SLAM2::MapPublisher* pMapPub)
        :mpSLAM(pSLAM),mpMapPub(pMapPub){}//new modified by eric.
...
int main(int argc, char **argv)
{
    ...
    MapPublisher* mpMapPub_ = new MapPublisher(SLAM.mpMap);//new added by eric.

    ImageGrabber igb(&SLAM, mpMapPub_);//new modified by eric.
...
void ImageGrabber::GrabStereo(const sensor_msgs::ImageConstPtr& msgLeft,const sensor_msgs::ImageConstPtr& msgRight)
{
    ...
    mpMapPub->Refresh();//new added by eric.
    // cerr << "  -----  map publisher has refreshed! -----  " << endl;
}
